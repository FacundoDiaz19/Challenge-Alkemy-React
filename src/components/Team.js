            ...
            {TeamHeros ? TeamHeros.map( (hero) => (
                    <div className="col">
                        ...
                        <div className="col-md-7 gx-2 text-center w-100 px-4 mb-2 btn-group">
                            <Link to={`/ViewHero/${hero.id}`} className='btn btn-outline-primary'>View Hero</Link>
                            <button className='btn btn-dark' onClick={() => DeleteHero(hero.id)}>Remove from the Team</button>
                        </div>
                        </div>
                    </div>
            )) : null}
